"""Stdlib-first fallback scanner for pytest-blackbox onboarding."""
    ...
    if owner is None and not (root / ".git").exists():
        for parent in root.parents:
            ...
            if (parent / ".git").exists():
                break
    nested = sorted(
        path for path in safe_paths(root, "pyproject.toml") if path != owner
    )
    return owner, nested
...
def package_manager_names(root: Path, data: Mapping[str, Any]) -> list[str]:
    names: set[str] = set()
    tool = data.get("tool", {})
    if (root / "uv.lock").is_file() or isinstance(tool, Mapping) and "uv" in tool:
        names.add("uv")
    if (root / "poetry.lock").is_file() or (
        isinstance(tool, Mapping) and "poetry" in tool
    ):
        names.add("poetry")
    if (root / "pdm.lock").is_file() or isinstance(tool, Mapping) and "pdm" in tool:
        names.add("pdm")
    if (root / "pixi.lock").is_file() or isinstance(tool, Mapping) and "pixi" in tool:
        names.add("pixi")
    return sorted(names)
            ...
            *(
                [
                    (
                        ...
                        "in a dedicated group (suggested: dev-ai)"
                    )
                ]
