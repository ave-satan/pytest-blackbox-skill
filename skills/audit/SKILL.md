...
Read [tooling.md](../../references/tooling.md). Run `../../scripts/lint_suite.py <project-root>` for mechanically proven diagnostics, then `../../scripts/audit_suite.py <project-root>` for the preserved semantic review and relevant collection, project lint, type, and test checks. Auto mode uses the enhanced toolchain only after dependency opt-in; bundled fallback remains supported without a managed install. Treat deterministic output as evidence, not proof of semantic completeness. Reconcile every emitted `SEM*` item rather than suppressing or dropping it. When available and authorized, use independent subagents for large functional surfaces; the primary agent retains ownership of the census, policy decisions, reconciliation, and final findings.
