...
Read the complete [shared policy](../../core/POLICY.md), [contracts reference](../../references/contracts.md), and [semantic reconciliation protocol](../../references/reconciliation.md) before acting. If `[tool.pytest-blackbox]` is absent, perform the shared onboarding workflow before changing tests. Read fixtures, repositories, infrastructure, and tooling references only when the requested behavior implicates them.

Keep the task plan aligned to these visible phases:

1. **Contract:** inventory the requested public/registered operation, run scoped contract drift across its complete owning component, and build the bidirectional scenario-evidence matrix from authoritative requirements. Reconcile old tests back to current authority; apply every relevant scenario dimension, complete-promise check, and counterfactual source condition. Resolve `ambiguous`/`unsourced` behavior before writing expected truth.
...
4. **Verify:** once the final scoped case set is known, reconcile function, category-file, and terminal-component names bottom-up; perform any local rename/split required for names to cover their complete assigned cases. Then rerun focused collection/execution and `audit_suite.py --scope <owning-test-component>` so deterministic and semantic reconciliation cover the selected contract surface without turning the task into a full-suite audit. Repeat `--scope` for every changed shared-support path and for another complete component only when the requested production change intentionally spans it. Run broader project checks proportional to risk and reconcile every matrix row to a truthfully named collected green node.
5. **Refactor:** improve implementation or test support only while the relevant suite remains green.
6. **Final:** rerun every check affected by refactoring and report the final matrix counts for `covered`, `partial`, `missing`, `ambiguous`, and `unsourced`, red/green evidence, exclusions, changed files, and remaining blockers. Do not claim the scope complete unless the last four counts are zero.
