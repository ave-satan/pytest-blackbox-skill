...
Inventory the requested contract surface, build the bidirectional transient evidence matrix, and read only the shared references implicated by the work. Before editing, run scoped contract drift across the complete owning component so old tests are reconciled back to current authority as well as new requirements forward to cases. Add the smallest complete set of public-boundary tests and support code; apply the scenario-dimension, complete-promise, and counterfactual passes before marking a row covered. Once the final scoped case set is known, reconcile function, category-file, and terminal-component names bottom-up; a local rename/split made necessary by the added cases is part of this task, not an unrelated suite refactor. Then run focused tests plus `audit_suite.py --scope <owning-test-component>` so deterministic and semantic reconciliation remain limited to the selected contract surface. Repeat `--scope` for every changed shared-support path and for another complete component only when this task intentionally changes it. Report the exact coverage boundary, authoritative evidence, and matrix counts by status.
