...
- A handler executed by a worker is mandatory; generic shared worker/runtime mechanics are a separate non-contract surface unless intentionally selected. Never let shared runtime coverage substitute for a handler contract. When worker runtime/registration is selected, keep its actual broker declaration and consumer registration contract in `test_topology.py`, separate from settlement and handler behavior.
...
- Validation varies one field at a time, except contractually related fields. For non-enum fields cover an ordinary valid value when appropriate, valid boundaries, the nearest invalid values outside them, and representative invalid forms. For enums and registered discriminators cover every publicly allowed member and at least one disallowed member instead of adding an ordinary randomized row. A broad transport/schema string bound does not create a successful boundary case when the application contract admits only registered discriminator values. Validation tests assert acceptance/rejection and the validation error—not downstream business work.
- Idempotency is never inferred. When an authoritative contract promises it, prove the first result, repeat with the same contractual identity, and prove the exact terminal result plus absence of duplicated direct artifacts. Put a cohesive idempotency matrix in `test_idempotency.py`.
...
- Tests receive purposeful repository fixtures, never raw storage clients or SQLAlchemy sessions. Ordinary database cases use repositories plus the same function-scoped transaction-owned `Connection`/`AsyncConnection` as the application. An explicitly enabled concurrency contract instead uses independently committing application transactions and fixture-owned deterministic cleanup, as defined in [infrastructure.md](../references/infrastructure.md).
...
3. Build or update the transient contract-evidence matrix before editing tests; map each discovered public or registered operation and every distinct public scenario/outcome class found in authoritative requirements and application-owned source behavior to a concrete collected test node. Generalized policy decisions may scope only non-contract surfaces and the recorded concurrency boundary; they never exempt a public contract. Operation presence alone never proves completeness.
...
6. Run `../scripts/lint_suite.py <project-root>` for deterministic diagnostics and `../scripts/audit_suite.py <project-root>` for the same diagnostics plus the preserved semantic operation-and-scenario completeness review, then focused and broader project checks as practical.
