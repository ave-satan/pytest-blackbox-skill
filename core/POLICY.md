...
Protect application-owned contracts through the real composed application. Treat public HTTP/JSON-RPC operations and every registered job, scheduler, and incoming-message handler as contract-bearing operations that require their own coverage.
...
- Use `pytest.raises` for expected exceptions and `pytest.fail` only for an explicitly forbidden reached branch. Preserve unexpected tracebacks.
...
- Cover every public HTTP/JSON-RPC operation and every registered job, scheduler, and incoming-message handler. Ordinary HTTP identity is method + path template; JSON-RPC identity is HTTP method + JSON-RPC method.
...
- Validation varies one field at a time, except contractually related fields. Cover an ordinary valid value, valid boundaries, the nearest invalid values outside them, representative invalid forms, every enum member, and an invalid enum member. Validation tests assert acceptance/rejection and the validation error—not downstream business work.
...
- Keep dependencies broad-to-narrow. Shared support never imports a functional group's private implementation.
...
- Keep case data function-scoped. Hoist only immutable/stateless factories and bootstrap. Reuse typed preparation contexts rather than rediscovering known actors, credentials, or resources.
...
- Ban sleeps, delayed polling, messaging timeouts, quiet windows, and retry backoff. Prefer deterministic completion; bounded immediate polling is a non-messaging fallback only when no completion signal exists.
