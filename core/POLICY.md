...
3. Build or update the transient matrix through [reconciliation.md](../references/reconciliation.md) before editing tests. Reconcile authoritative requirements to collected cases and every existing case back to authority; perform the applicable scenario-dimension, complete-promise, counterfactual-source, and scoped contract-drift passes. Inspect application-owned source only for candidates, never expected truth. Generalized policy decisions may scope only non-contract surfaces and the recorded concurrency boundary; they never exempt a public contract. After the final scoped case set is known, reconcile names against all assigned rows. Do not claim completeness while any row is `partial`, `missing`, `ambiguous`, or `unsourced`.
