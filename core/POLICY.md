...
For authorized application behavior, use `develop`: derive the complete scoped contract matrix from authoritative requirements and public outcomes, reconcile it in both directions, write its black-box cases first, obtain a meaningful behavior-level red, implement the smallest cohesive production change, and finish with focused green plus scoped semantic reconciliation. Registration-only collection/`404` failures prove only registration; add only an approved real composition skeleton before seeking assertion-level red. Unexpectedly green new tests require investigation before production edits.
...
- A focused change inventories only its relevant surface; a full audit inventories the complete registered/reachable surface. Before a completeness claim, use the bidirectional transient evidence protocol in [reconciliation.md](../references/reconciliation.md), including authoritative requirement references, natural positive/negative artifacts, collected nodes, and explicit `covered`/`partial`/`missing`/`ambiguous`/`unsourced` status—not project configuration.
...
- Treat `covered` as a verified complete promise: each independently breakable response, created/changed item, and required absence/preservation must have a distinguishing assertion or artifact read. For paginated work, prove all relevant pages with the cheapest supported small-page setup and complete collection inspection, not a large default-page fixture plus a count and last-item check.
...
- Surface a conflict between an accepted project exclusion and a mandatory public/registered contract before changing tests or claiming completeness. A focused/excluded registry entry never resolves that conflict silently; request an authoritative scope decision and continue unaffected work.
...
- Run all database migrations and mandatory reversal in-process; failures remain visible while cleanup continues. Neither collected tests nor fixtures/environment code check data preservation or portability across migration revisions: do not seed a pre-migration schema and compare rows before/after upgrade, or move such assertions into bootstrap. Broker topology runs only real forward bootstrap and resource deletion.
...
3. Build or update the transient matrix through [reconciliation.md](../references/reconciliation.md) before editing tests. Reconcile authoritative requirements to collected cases and every existing case back to authority; perform the scope-conflict, scenario-dimension, complete-promise, counterfactual-source, and scoped contract-drift passes. Inspect application-owned source only for candidates, never expected truth. Generalized policy decisions may scope only non-contract surfaces and the recorded concurrency boundary; they never exempt a public contract. After the final scoped case set is known, reconcile names against all assigned rows. Do not claim completeness while any row is `partial`, `missing`, `ambiguous`, or `unsourced`, any mandatory-scope conflict remains unresolved, or applicable semantic review is unfinished.
...
8. Report all five matrix status counts, coverage boundary/exclusions, changed files, commands, pass counts, warnings, manual-review items, and blockers.
