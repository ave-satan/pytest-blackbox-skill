...
- Use partial structural comparison only for a variation/edge case whose remaining elements are already protected, or for an intentionally extensible natural container whose declared subset is the complete application-owned contract. Pass the complete observed value to a matcher that requires every declared element and permits unrelated elements. Do not rebuild a selected-field projection or delete, merge, or overwrite ignored values. If equality requires normalization, normalize the complete container without selecting fields. Closed primary values remain exact.
...
- Name every public domain-facing test-support type, fixture, and method by its role plus the domain action, target, state, or outcome visible to the test. Names truthfully describe the returned or mutated artifact and its cardinality; storage, transport, discriminator values, caller scenarios, and implementation optimizations stay private. A generic role method is allowed only when its owning component has one unambiguous contract. Reusable structural/protocol primitives instead use a precise role/semantic name at their intentionally generic abstraction level and never invent a domain noun.
