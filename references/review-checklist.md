...
- Were accepted project exclusions and every matching coverage selector compared with the final registered operation census? If one conflicts with mandatory public/registered coverage, was the conflict reported for an authoritative decision rather than silently treated as permission or ignored?
...
- Would changing only a production constant/default/DTO/codec/private algorithm leave the test-owned expected value unchanged, proving that actual and expected cannot drift together? When only an invariant is promised, does the test avoid cloning the current selection/hash/ranking algorithm?
...
- Was the bidirectional transient matrix reconciled through [reconciliation.md](reconciliation.md), including every operation matched by a `focused` rule, every authoritative requirement mapped to a collected node, and every existing behavior test mapped back to precise authority?
- Does each row record the natural response/observation, changed artifacts, explicitly promised absent/unchanged artifacts, and one of `covered`, `partial`, `missing`, `ambiguous`, or `unsourced`? Are all five status counts reported, with no completeness claim while the last four are nonzero?
- For every `covered` row, can the author point to the collected assertion/artifact read that would fail if each independently breakable promise alone regressed? Are unresolved scope conflicts or semantic review items kept visible outside the five counts?
...
- Was scenario completeness checked semantically from authoritative requirements plus source candidates, explicitly classifying actor/owner isolation, lifecycle state, compound identity, time/local-date/timezone/TTL, configuration, external-call snapshot/post-call guards, async ownership phases, batch partitions, repetition, and enabled concurrency as applicable or not applicable—without manufacturing a Cartesian product or corrupt states? When ownership affects selection/access, does each operation cover the same resource owned by another principal?
- Was each source condition that can change a public result or direct artifact checked counterfactually by deletion, inversion, wrong-actor scope, or movement across an external call, with missing protection recorded instead of assuming nearby line execution is coverage?
- Does every covered scenario distinguish all independently breakable promises, including exact positive artifacts and contractually absent or preserved effects, without asserting internal calls or manufacturing one aggregate observation?
- Does every bulk/paginated case cross a real page/batch boundary with minimal supported setup and inspect all relevant items, not just the reported count and one representative/last item? Was a small supported page size preferred over a large fixture, without patching the application or replacing the store?
- Does every asserted undesirable outcome represent a required contract rather than merely an accepted possibility/risk? Would a safer implementation that eliminates that outcome still satisfy the test?
- For a changed scope, was contract drift checked for added, changed, removed/disabled, renamed/aliased, and async-phase-moved behavior so contradictory old cases were not silently retained?
...
- Are runtime legacy-format/backward-compatibility cases present only for an authoritative product/operational promise and exercised through the already migrated environment and a public boundary? Are schema migrations limited to fixture-owned upgrade/reversal calls and their failures, with no old-schema data seeding, pre/post-migration comparison, preservation assertion, or conversion check hidden in collected tests, fixtures, or environment helpers?
...
- If an observational runtime probe is used, is it classified separately from a performance double, limited to selected runtime scheduling/settlement evidence through a supported seam, deterministic, reset per case, and never substituted for handler-domain contract coverage?
...
- Were library call defaults inspected so an apparently no-wait call does not inherit a positive timeout, and were test-only retry/backoff/timeout settings explicitly disabled or minimized instead of relying on production defaults?
...
- Did `scripts/lint_suite.py` run successfully, and did `scripts/audit_suite.py` preserve and reconcile every applicable semantic item including operation census, forward/reverse authority mapping, scenario dimensions, complete promises, independent encoding, hidden waits, and counterfactual checks in `SEM001`/`SEM006`/`SEM013`–`SEM017`?
