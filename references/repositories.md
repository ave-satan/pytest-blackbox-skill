...
Repository classes are ordinary allowed support classes under `tests/repositories/`; they are not pytest test classes. In ordinary cases, public fixtures constructing them live in `tests/fixtures/` and depend on the relevant private isolated client or on the database stack's function-scoped rollback boundary. For SQLAlchemy this is the exact `Connection`/`AsyncConnection` yielded by `_database_transaction`. Explicit concurrency cases use only the separately named committed-state repository adapters described later in this section and composed according to [fixtures.md](fixtures.md) and [infrastructure.md](infrastructure.md).

When the project uses SQLAlchemy, do not import or use `Session`, `AsyncSession`, `sessionmaker`, or `async_sessionmaker` anywhere in test functions or test support. Database repositories execute SQLAlchemy statement objects through `Connection.execute` or `AsyncConnection.execute`, matching the project's concurrency model: `select()`, `insert()`, `update()`, and `delete()`. Do not use ORM unit-of-work methods such as `add`, `add_all`, `flush`, `refresh`, relationship mutation, or session query APIs. The production application may keep its own ORM session internally; ordinary test composition passes only the outer connection through a production-owned binding hook, while the explicit concurrency composition follows the committed-connection carve-out below. For another database stack, use its supported low-level statement/query and transaction API with the same repository-only test boundary; do not imitate SQLAlchemy types or introduce an unsupported production seam merely to match this example.
...
In ordinary cases, database repositories use the same transaction-owned rollback boundary as the application and never independently commit, roll back, begin another transaction, open a separate client/connection, or escape the transaction. With SQLAlchemy, use the same `Connection`/`AsyncConnection` and build statements against mapped tables. Return explicit immutable DTOs/records for multi-column domain results and scalars for counts, existence checks, identifiers, and primitive projections—never generic database row mappings or session-bound ORM instances. Return a mapping only when that mapping is itself the explicit stored domain value, such as JSON/JSONB metadata. This example is SQLAlchemy-specific:
...
When—and only when—`test_concurrency = true` and an authoritative race contract is under test, expose separately named concurrency repository fixtures backed by a private connection factory. Each arrange mutation runs in and commits its own short transaction; each artifact read uses a fresh transaction so independently committed application results are visible. The repository still exposes typed domain CRUD rather than connections/transactions, and the owning fixture deletes all committed case artifacts deterministically. Never give raw connection factories to tests, reuse this adapter in ordinary cases, or make ordinary repository `create` commit.
...
Natural independent observations receive direct assertions. A separately bound checked observation uses `actual`/`actual_*`; arrange-only entities keep domain names. An already compound artifact receives one comparison: exact when closed, or a partial structural matcher only for an intentionally extensible natural container with a complete application-owned subset or a variation/edge case whose remaining elements are already protected. Never rebind independent observations to generic `result` or manufacture a combined object only to merge database, cache, object, status, or body assertions.
