...
Invoke migration lifecycle operations from the session fixture owning the migrated database. Apply every migration before application startup; when the application has a migration stack, its complete configured reverse lifecycle is mandatory at teardown before deleting the database. Use that framework's programmatic API—Alembic's example above is illustrative, not a required dependency. Do not swallow a downgrade/reversal failure: keep it as a visible suite error while still attempting resource cleanup. If the project intentionally contains an irreversible migration or its framework exposes no safe reverse operation, report that incompatibility explicitly instead of silently skipping it or falling back to CLI/subprocess. A successful lifecycle call and visible failure are the only migration checks in this suite. Do not add a standalone migration test or transplant its setup/assertions into fixtures or `tests/environment/`: neither tests nor bootstrap seed old-revision rows and compare data before/after migration, assert preservation, or verify transformation/portability between schema revisions. Remove such a test from the suite instead of hiding it in setup.

Do not confuse that lifecycle invariant with an application-level promise to read a legacy format at runtime. Cover runtime backward compatibility only when an authoritative product or operational requirement explicitly promises it and the input can be arranged in the already migrated test environment through a domain repository or file/data builder. Invoke the supported public application boundary and assert only the public outcome/direct artifacts. Never downgrade to an old schema, seed rows before an upgrade, compare pre/post-migration data, inspect raw keys/table layout, or assert the conversion algorithm as a workaround. If the promise cannot be tested without exercising migration data transfer, report that scope limitation instead of embedding the check in a fixture. Without an authoritative compatibility requirement, an implementation branch that recognizes old data is not enough reason to create or retain a test.
