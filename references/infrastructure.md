...
Do not confuse that lifecycle invariant with an application-level promise to read or transform legacy data at runtime. Cover runtime backward compatibility only when an authoritative product or operational requirement explicitly promises it. Arrange the legacy state through a domain repository or file/data builder, invoke the supported public application boundary, and assert the preserved public outcome/direct artifacts. Name the test after that behavior, not after “migration”; never inspect raw keys, table/index layout, hashes, storage references, or the conversion algorithm. Without an authoritative compatibility requirement, an implementation branch that recognizes old data is not enough reason to create or retain a test.
