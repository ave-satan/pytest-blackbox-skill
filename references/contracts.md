...
When one natural value is already compound, compare that entire value with one equality assertion. The primary contract compares the exact complete public value: a PATCH or partial input does not make unchanged stable response fields optional. Exact dictionaries enforce key sets; exact lists enforce length, order, and nesting. Do not split one response body/model/list into an assertion per member. Native Python is the default: use a dictionary for an exact mapping, a list for an exact ordered sequence, `len` for exact cardinality, and a direct predicate/comprehension for a focused property. Use a matcher only when it contributes dynamic constraints, order-insensitive multiplicity, partial-container semantics, object/DTO equality, or materially clearer compound diagnostics.

Partial structural comparison has exactly two valid scopes: a variation/edge case intentionally checking selected elements while the primary contract protects the rest, or an intentionally extensible natural container where the declared subset is the complete application-owned contract and framework/transport additions are not. HTTP headers are a common example of the second scope. Keep the observed compound value intact and compare it once with a pure structural matcher describing the required elements. The public matcher name must expose partial semantics—prefer `PartialMapping`/`PartialObject` or a coherent equivalent, never `Mapping(partial=True)`/`Object(partial=True)`. The matcher must reject a missing declared element, compare its value through literals or nested matchers, and permit unrelated elements outside the declared subset. Do not union defaults into the actual value, remove keys, or overwrite dynamic values merely to make equality pass. A closed primary response body/model/list remains an exact complete comparison.

A focused projection is different from partial structural matching: it asserts a single derived property of a collection rather than pretending to compare each whole element. When order of one already-protected field is the case, prefer `assert [item["id"] for item in actual_items] == expected_ids` to `OrderedList([PartialMapping(id=...) ...])`. Keep this exception narrow: the primary case still exact-compares complete items, and the projection must not become a way to omit unprotected stable fields from a primary contract.

Some natural containers implement equality without delegating to an expected-side matcher. Convert such a container in full to its natural normalized representation before comparison—for example, all HTTP headers to a mapping—without selecting only the asserted elements during conversion. Keep protocol-specific multiplicity when normalization would otherwise collapse it. This compatibility step does not authorize manufacturing a new aggregate or weakening the complete primary contract.
...
Use exact literals and native operators whenever possible. Use `OrderedList` when order is contractual and matcher composition/diagnostics materially help; otherwise an ordinary list is clearer. Use `UnorderedList` only when order is not contractual. It preserves exact length and duplicate multiplicity and performs one-to-one matching; never implement it with sets, heterogeneous sorting, or greedy broad matches. Do not introduce matcher classes for plain length, membership, or exclusion checks that read more directly as `len`, `in`/`not in`, `all`, or a comprehension.

For object/DTO actual values, use a test-owned object matcher such as `Object(...)` when dynamic fields prevent exact construction. It compares the object as one value, checks the advertised test-owned/public type constraint when one is intentionally supplied, and compares an explicit exact attribute set with nested literals/matchers. Partial attribute matching uses an explicitly partial type such as `PartialObject(...)`, not a boolean mode on the exact matcher, and is allowed only for an intentionally extensible public contract whose declared subset is complete or for a focused variation/edge case whose remaining stable attributes are already protected by the primary exact contract. The matcher never constructs, validates, serializes, or decodes through a production DTO/schema.
