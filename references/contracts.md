...
Partial structural comparison has exactly two valid scopes: a variation/edge case intentionally checking selected elements while the primary contract protects the rest, or an intentionally extensible natural container where the declared subset is the complete application-owned contract and framework/transport additions are not. HTTP headers are a common example of the second scope. Keep the observed value intact and compare it once with a pure structural matcher describing the required elements. The matcher must reject a missing declared element, compare its value through literals or nested matchers, and permit unrelated elements outside the declared subset. Do not construct a dictionary/list/DTO from selected actual fields, union defaults into the actual value, remove keys, or overwrite dynamic values merely to make equality pass. A closed primary response body/model/list remains an exact complete comparison.
