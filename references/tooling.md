...
- `MIG001` rejects a collected test that directly invokes Alembic's upgrade/downgrade API or a test-environment migration lifecycle helper. Its correction is to remove the standalone migration test, not relocate its legacy-data setup or before/after assertions into a fixture. Fixture-owned bootstrap and reversal only run migrations and surface lifecycle failures; runtime backward-compatibility, when promised and arrangable in the migrated environment, remains a separate public-boundary contract.
...
- `VAL001` rejects parametrized expected status or a literal/`lambda`-returned `None` error sentinel in `test_validation.py`; acceptance and rejection belong to separate homogeneous functions with their outcome asserted directly. Less mechanically obvious mixed outcome shapes remain under `SEM012`.
...
Manual findings never appear in `lint_suite.py`. They remain in the `Semantic review required` section of `audit_suite.py`. Without scope, `SEM001` requires the complete operation/evidence matrix from final framework registrations and actions, policy-conflict review, plus bottom-up reconciliation of the final case set against category and terminal-component names; `SEM006` requires a separate complete-suite forward scenario/outcome pass. `SEM013` requires the reverse test-to-authority pass and explicit `covered`/`partial`/`missing`/`ambiguous`/`unsourced` counts, distinguishing required outcomes from tolerated risks. `SEM014` owns applicable scenario dimensions and counterfactual source-condition checks, especially actor isolation, time/TTL, and state across external-call or async phase boundaries. `SEM015` requires every covered row to distinguish all independently breakable positive, absent, preserved, and bulk/paginated artifacts using a complete relevant collection. `SEM016` traces every input and expected value back to independent test-owned data instead of production configuration/types/defaults/private algorithms. `SEM017` reviews baseline application-configurable retry/backoff defaults and hidden library timeouts. `SEM018` reviews public invocation boundaries versus direct internal helpers/coordinators/migration lifecycle calls and checks fixtures/environment code for hidden migration data-transfer assertions. With scope, these items apply only to the selected public operations/components, and conditional scheduler/worker/WebSocket/validation items appear only when their surface intersects the selected paths. The agent still derives semantic truth from authoritative requirements and registration/source evidence—the path filter does not prove completeness by itself. `SEM010` reviews native-first assertions, explicitly named partial matchers, and the narrow ordered scalar-projection exception without binding policy to one implementation. `SEM011` reviews shortest truthful owner-relative names, canonical operations, typed domain results, response-oracle module/callable names, and support ownership boundaries across public domain-facing test support while allowing precisely named generic structural/protocol primitives. `SEM012` reviews per-field homogeneous acceptance/rejection parametrization without status/error sentinels or outcome branches. If the tests root is absent or cannot be inspected, the runner reports that structural blocker instead of pretending to emit a complete semantic checklist. Complete-suite `SEM002` follows focused registry selections and now calls out mandatory-scope conflicts; scoped workflows instead reconcile the selected operation against its applicable registry decision explicitly. `SEM003` follows schedulers; `SEM004` covers mandatory handler contracts independently from shared worker runtime; conditional `SEM008` covers selected shared runtime/settlement behavior and classifies observational probes separately from performance doubles; `SEM009` verifies that broker topology is a private session-bootstrap invariant rather than a collected test surface and has no inverse-topology teardown. `SEM005` follows detected WebSocket test support/surfaces and includes current-contract-only outcome projections plus Transport/Connection/Client ownership; `SEM007` appears only when `test_concurrency = true`. A clean deterministic section does not satisfy these items or prove coverage completeness. Test-first chronology is execution evidence owned by the `develop` workflow and is intentionally not inferred from the final tree.

Deterministic checks now also flag three common semantic-support leaks when their visible shape is conclusive: `ORC001` rejects production `Settings` values used to derive expected assertions/builders; `TIME002` rejects unconstrained expected-side date/time matchers; and `WAIT003` rejects messaging drains that request `fail=False` but silently inherit a positive library timeout. These checks prove only the visible form and do not replace the corresponding matrix review.
